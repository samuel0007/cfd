...
public:
    ...
    auto as_range()
    {
        return std::ranges::views::transform(matrix, [this](auto &row)
                                             { return row[col]; });
    }
...
namespace utils
{
    ...
    value_t conservativeToPrimitive2D(double gamma, value_t u)
    {
        return {
            u[0],
            u[1] / u[0],
    ...
    vvalue_t conservativeToPrimitive(double gamma, vvalue_t solution)
    {
        ...
        for (size_t i = 0; i < nx; ++i)
        {
            ...
            {
                res[i][j] = conservativeToPrimitive2D(gamma, solution[i][j]);
            }
        }
        return res;
    }
...
class SourceEquation2D
{
public:
    SourceEquation2D(size_t nx, size_t ny, ProblemT &problem) : _nx(nx), _ny(ny), _problem(problem)
    {
        _source.resize(_nx + 2);
        for (auto &row : _source)
        {
            row.resize(_ny + 2);
        }
    };

    vvalue_t &getSource(const vvalue_t &u, const vpoint2D_t cells, double dt)
    {
        for (size_t i = 1; i < this->_nx + 1; ++i)
        {
            for (size_t j = 1; j < this->_ny + 1; ++j)
            {
                _source[i][j] = this->_problem.evaluateSource(u[i][j], cells[i][j]);
            }
        }
        return _source;
    }
...
public:
    ...
    v1Dvalue_t &getFlux(std::ranges::input_range auto const &u, double dt)
    {
...
struct EOS
{
    ...
    value_t primitiveToConservative(value_t u)
    {
        return static_cast<Derived &>(*this)._primitiveToConservative(u);
    }

    value_t conservativeToPrimitive(value_t u) 
    {
        return static_cast<Derived &>(*this)._conservativeToPrimitive(u);
    };

    vvalue_t conservativeToPrimitive(vvalue_t solution)
    {
        const size_t nx = solution.size();
        const size_t ny = solution[0].size();
        vvalue_t res(nx);
        for (size_t i = 0; i < nx; ++i)
        {
            ...
            for (size_t j = 0; j < ny; ++j)
            {
                res[i][j] = this->conservativeToPrimitive(solution[i][j]);
            }
        }
        return res;
    }
...
struct IdealGas: EOS<IdealGas>
{
    double gamma;
    IdealGas(double gamma_) {
        this->gamma = gamma_;
    }

    value_t _conservativeToPrimitive(value_t u) {
        return {
            u[0],
            u[1] / u[0],
            u[2] / u[0],
...
template <typename DIRECTION, typename EOS_IMPL>
class EulerProblem1D
{
public:
    explicit EulerProblem1D(EOS<EOS_IMPL> eos, double gamma, double eps) : _eos(eos), _gamma(gamma), _eps(eps)
    {
        this->_max_it = 100;
    };
    ...
    value_t RiemannSolver(const value_t &u0, const value_t &u1)
    {
        ...
        const value_t uL = utils::conservativeToPrimitive2D(this->_gamma, u0);
        const value_t uR = utils::conservativeToPrimitive2D(this->_gamma, u1);
    ...
    value_t evaluateSource(value_t u, point2D_t cell_centroid)
    {
        // return {0., 0., 0., 0.};
        const auto &r = cell_centroid[0];
        const auto &z = cell_centroid[1];

        const auto &q = _eos.conservativeToPrimitive(u);
...
template <typename EOS_IMPL>
class F : public EulerProblem1D<F, EOS_IMPL>
{
public:
    value_t _f(value_t u)
    {
        auto q = this->_eos.conservativeToPrimitive2D(u);
        const auto &rho = q[0];
        const auto &vx = q[1];
        const auto &vy = q[2];
        const auto &p = q[3];
...
template <typename EOS_IMPL>
class G : public EulerProblem1D<G, EOS_IMPL>
{
public:
    value_t _f(value_t u)
    {
...
public:
    ...
    Mesh(int nx, int ny, double min_x, double max_x, double min_y, double max_y) : _nx(nx), _ny(ny), _min_x(min_x), _max_x(max_x), _min_y(min_y), _max_y(max_y), _dx((max_x - min_x) / (double)nx), _dy((max_y - min_y) / (double)ny)
    {
...
{
public:
    Simulation(double final_time, ConservationEquation1D<P1, F1> &eq_f, ConservationEquation1D<P2, F2> &eq_g, SourceEquation2D<P1> &s, Mesh mesh, std::function<value_t(point2D_t)> ic, InSituVis &vis) : _final_time(
                                                                                                                                                                                                                    final_time),
                                                                                                                                                                                                                _eq_f(eq_f), _eq_g(eq_g), _s(s), _mesh(mesh), _vis(vis)
    {
        ...
        this->solution[nx + 1][ny + 1] = bc(this->solution[nx][ny]);
        this->solution[nx + 1][0] = bc(this->solution[nx][0]);
        this->solution[0][ny + 1] = this->solution[0][ny];

        for (int j = 1; j < ny + 1; ++j)
        {
            this->solution[0][j] = this->solution[1][j];
            this->solution[nx + 1][j] = bc(this->solution[nx][j]);
        }

        for (int i = 1; i < nx + 1; ++i)
        {
            this->solution[i][0] = bc(this->solution[i][1]);
            this->solution[i][ny + 1] = bc(this->solution[i][ny]);
        }
    };
    ...
    bool evolve()
    {
        ...
        // Transmissive BCs
        this->solution[0][0] = bc(this->solution[1][1]);
        ...
        // U balance update
        vvalue_t &source = this->_s.getSource(solution, this->_mesh.getFullCellPoints(), min_dt);
        for (size_t i = 1; i < this->nx + 1; ++i)
        {
            for (size_t j = 1; j < this->ny + 1; ++j)
            {
                solution[i][j] = solution[i][j] + min_dt * source[i][j];
            }
        }
    ...
    void plot2d()
    {
        ...
        std::vector<float> density(2 * nx * ny);
        std::vector<float> velocity_x(2 * nx * ny);
        std::vector<float> velocity_y(2 * nx * ny);
        std::vector<float> pressure(2 * nx * ny);

        for (int i = 0; i < nx; ++i)
        {
            for (int j = 0; j < ny; ++j)
            {
                density[i + j * 2 * nx] = primitive_values[nx - i - 1][j][0];
                velocity_x[i + j * 2 * nx] = primitive_values[nx - i - 1][j][1];
                velocity_y[i + j * 2 * nx] = primitive_values[nx - i - 1][j][2];
                pressure[i + j * 2 * nx] = primitive_values[nx - i - 1][j][3];
            }
        }
        ...
        for (int i = 0; i < nx; ++i)
        {
            for (int j = 0; j < ny; ++j)
            {
                density[(i + nx) + j * 2 * nx] = primitive_values[i][j][0];
                velocity_x[(i + nx) + j * 2 * nx] = primitive_values[i][j][1];
                velocity_y[(i + nx) + j * 2 * nx] = primitive_values[i][j][2];
                pressure[(i + nx) + j * 2 * nx] = primitive_values[i][j][3];
            }
        }
        ...
        plt::subplot(2, 2, 1);
        plt::imshow(pP, ny, 2 * nx, 1);

        plt::subplot(2, 2, 2);
        plt::imshow(pD, ny, 2 * nx, 1);

        plt::subplot(2, 2, 3);
        plt::imshow(pVx, ny, 2 * nx, 1);

        plt::subplot(2, 2, 4);
        plt::imshow(pVy, ny, 2 * nx, 1);
    ...
    void plot(double blim = -1.0, double tlim = 1.0)
    {
        ...
        // TODO
        std::ranges::transform(solution, primitive_values.begin(), [&](value_t u)
                               { return utils::conservativeToPrimitive2D(1.4, u); });
        std::ranges::transform(primitive_values, density.begin(), [&](value_t u)
                               { return u[0]; });
        std::ranges::transform(primitive_values, velocity_x.begin(), [&](value_t u)
                               { return u[1]; });
        std::ranges::transform(primitive_values, velocity_y.begin(), [&](value_t u)
                               { return u[2]; });
        std::ranges::transform(primitive_values, pressure.begin(), [&](value_t u)
                               { return u[3]; });
    ...
    void plot_insitu(size_t index) {

        const size_t nx = solution.size() - 2;
        const size_t ny = solution[0].size() - 2;

        vvalue_t primitive_values = _eq_f.getEOS().::conservativeToPrimitive(1.4, solution);
        std::vector<double> density(2 * nx * ny);
        std::vector<double> velocity_x(2 * nx * ny);
        std::vector<double> velocity_y(2 * nx * ny);
        std::vector<double> pressure(2 * nx * ny);

        for (int i = 0; i < nx; ++i)
        {
            for (int j = 0; j < ny; ++j)
            {
                density[i + j * 2 * nx] = primitive_values[nx - i][j+1][0];
                velocity_x[i + j * 2 * nx] = primitive_values[nx - i][j+1][1];
                velocity_y[i + j * 2 * nx] = primitive_values[nx - i][j+1][2];
                pressure[i + j * 2 * nx] = primitive_values[nx - i][j+1][3];
            }
        }
        ...
        for (int i = 0; i < nx; ++i)
        {
            for (int j = 0; j < ny; ++j)
            {
                density[(i + nx) + j * 2 * nx] = primitive_values[i+1][j+1][0];
                velocity_x[(i + nx) + j * 2 * nx] = primitive_values[i+1][j+1][1];
                velocity_y[(i + nx) + j * 2 * nx] = primitive_values[i+1][j+1][2];
                pressure[(i + nx) + j * 2 * nx] = primitive_values[i+1][j+1][3];
            }
        }

        _vis.write(density, velocity_x, velocity_y, pressure, index);
    }
    vvalue_t solution;
...
int main(int argc, char *argv[])
{
    ...
    InSituVis vis(nx, ny, mesh.getDx(), mesh.getDy());
    IdealGas eos(gamma);
    EulerProblem1D<F<IdealGas>, IdealGas> problem_F(eos, 1.4, 1e-10);
    EulerProblem1D<G<IdealGas>, IdealGas> problem_G(eos, 1.4, 1e-10);

    NumericalFlux::FluxPolicy<NumericalFlux::FORCE<EulerProblem1D<F<IdealGas>, IdealGas>>, EulerProblem1D<F<IdealGas>, IdealGas>> fluxPolicy_F(problem_F);
    NumericalFlux::FluxPolicy<NumericalFlux::FORCE<EulerProblem1D<G<IdealGas>, IdealGas>>, EulerProblem1D<G<IdealGas>, IdealGas>> fluxPolicy_G(problem_G);
    ...
    SourceEquation2D euler_source_term(nx, ny, problem_F);

    Simulation sim(final_time, equation_f, equation_g, euler_source_term, mesh, utils::test3_2d, vis);
    // sim.evolve();
    // sim.plot();
    // sim.evolve();
